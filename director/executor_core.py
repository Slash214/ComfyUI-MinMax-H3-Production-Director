...
from .segment_memory import (
    _pixel_exports_complete,
    _release_segment_pixels,
    should_release_segment_pixels,
)
...
def _build_minimax_inputs(
    ...
    *,
    ...
    long_px = ref_image_long_preset_px(resolve_ref_image_size(seg, plan)) if ref_images else None
...
def execute_director_plan_core(
    ...
    *,
    ...
    low_memory_segment_export: bool = False,
) -> tuple[
    torch.Tensor,
    list[torch.Tensor],
    list[dict[str, Any]],
    ...
    release_pixels = should_release_segment_pixels(
        plan.export_mode, low_memory_segment_export, confirm_first_pass_enabled(plan)
    )
    persisted_segments: set[int] = set()
    released_segments: set[int] = set()

    def record_pixel_exports(index, paths, *, require_pre):
        # Refresh proof after every write, including continuity-trim rewrites.
        # Old files must not make a failed rewrite look successful.
        persisted_segments.discard(index)
        if _pixel_exports_complete(mp4_run_dir, index, paths, require_pre=require_pre):
            persisted_segments.add(index)

    if low_memory_segment_export and not release_pixels:
        reports.append("Low-memory segment export inactive: requires segments mode and first-pass confirmation OFF.")
    ...
    def _run_one_segment(
            ...
            if prev_export_trim > 0:
                ...
                if prev_chunk is not None:
                    ...
                    if prev_seg is not None:
                        ...
                        persisted_segments.discard(prev_idx)
                        if hold_after_first:
                        ...
                        record_pixel_exports(
                            prev_idx, mp4_paths,
                            require_pre=(
                                completed_pre_refine.get(prev_idx) is not None
                                and completed_pre_refine[prev_idx] is not prev_chunk
                            ),
                        )
        ...
        # Multi-segment runs may retain the previous conditioning/decoder
        # workspace. Route cleanup through the RAM-aware policy; single-shot
        # runs only need the targeted text-encoder release below.
        ...
        # balanced_20gb: hold the pre-refine RGB decode until after refine so the
        # H3 stages stay contiguous. Gated on run_refine, not will_refine — when
        # the user asked to confirm the first pass, the frames are the whole
        # point and must not be deferred.
        defer_pre_decode = (
        ...
        if run_refine:
            ...
            # pass, and under balanced_20gb nothing has been unloaded. Capture
            # what is resident and how many tokens are about to be attended over.
        ...
        mem.track_copy(
            label="chunk",
            op="decoded -> conditional cpu/float32",
            src=decoded,
            result=chunk,
        )
        if pre_export is not None:
            ...
            mem.track_copy(
                label="pre_chunk",
                op="pre_export -> conditional cpu/float32",
                src=pre_export,
                result=pre_chunk,
            )
        else:
        ...
        # Confirmation hold reuses `chunk` as the pre-refine output; clone it so
        # the two outputs are not the same tensor. Must happen before the
        # balanced release below drops `decoded`.
        ...
        record_pixel_exports(seg.index, mp4_paths, require_pre=run_refine and pre_chunk is not chunk)
    ...
    for seg in all_segments:
        ...
        if seg.index in run_indices:
            ...
            if release_pixels and seg.index > 0:
                ...
                if _release_segment_pixels(
                    ...
                    released_segments.add(seg.index - 1)
            if plan.export_mode == "all":
    ...
    if export_segments_mode:
        ...
        reports.append("Export mode: segments — no full-timeline concat; full IMAGE batches retained unless released below.")
        if release_pixels:
            reports.append(
                "Low-memory segment export: released segments "
                + (", ".join(str(i + 1) for i in sorted(released_segments)) or "none")
                + "; those internal posters are omitted from IMAGE / pre-refine outputs. "
                "Segments without confirmed MP4 exports retain full frames."
            )
    else:
