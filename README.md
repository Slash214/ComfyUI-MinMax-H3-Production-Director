...
最新更新：**2026-09-10 — WebP 动态采样预览、可选时间分块和二采空间分块**。
查看[完整更新说明与使用方法](CHANGELOG.md)。

## 20GB Ampere 面对的是什么

本分支已合入上游截至 `5fede41`（2026-09-10 检查）的更新：执行期音频缓存与按需解码、
...
开启「段间引导」后可选择「引导＋重绘」，重绘幅度为 0.40–0.95，默认 0.65；
幅度越大，对上段尾部的重绘越多。原有「引导」仍为默认，新模式不会在二采重复锁定前缀。
